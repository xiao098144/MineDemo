		...
		public void onReceive(Context context, Intent intent) {
			...
			if (UsbManager.ACTION_USB_DEVICE_DETACHED.equals(action)) {
				synchronized (this) {
					...
					if (device != null) {
						File file = GetFiles(Environment.getRootDirectory().getParentFile().getAbsolutePath());
						if (file.isFile()) {
							tv_result.setText(" 搜索结果  "+file.getAbsolutePath());
						} else {
							File[] files = file.listFiles();
							tv_result.setText(" 搜索结果  "+file.getAbsolutePath()+"\n");
							for (int i = 0; i < files.length; i++) {
								tv_result.append("\n 文件名 "
										+ files[i].getName() + " 文件路径 "
...
//							tv_source.append("\nUDISK "
//									+ udisk_file.getAbsolutePath());
//							File[] files = udisk_file.listFiles();
//							if (files != null) {
//								tv_source.append("\nUDISK下存在" + files.length
//										+ "个文件");
//								for (int i = 0; i < files.length; i++) {
//									tv_result.append("\n 文件名 "
//											+ files[i].getName() + " 文件路径 "
...
//							}
	...
	public void readUdisk() {
		File[] files = Environment.getRootDirectory().getParentFile()
				.listFiles();
	...
	public File GetFiles(String path) 
	{
		...
		for (int i = 0; i < files.length; i++) {
			...
			if (f.isDirectory() && f.getPath().indexOf("/.") != -1) {
				...
				if (f.getName().startsWith("u") || f.getName().startsWith("storage")) {
			...
			} else if (f.isFile() || (f.isDirectory() && f.getPath().indexOf("/.") == -1)) {// 忽略文件或者
				continue;
			}
		}
		return null;
		...
		if (listFiles != null) {
			for (int i = 0; i < listFiles.length; i++) {
				...
				tv_result.append("\n" + file.getAbsolutePath() + "   "
						+ file.getName());
				spUtil.setString(i+"", file.getAbsolutePath() + "   "
						+ file.getName());
				if (file.isDirectory()) {
					if (isBreak) {
						break;
...
>>>>>>> bf09fe159d1667e6033f20222f87b86b176a663b
